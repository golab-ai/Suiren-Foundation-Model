...
class EdgeDegreeEmbedding(torch.nn.Module):
    ...
    def forward(
        self, atomic_numbers, edge_distance, edge_index, num_nodes, node_offset=0, ssp=False
    ):
        if ssp:
            task_idx = 1
        else:
        ...
        x_edge_embedding._rotate_inv(self.SO3_rotation[task_idx], self.mappingReduced)
